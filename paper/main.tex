...
LLM explanation generation is the main bottleneck ($\sim$10 minutes for 25k examples on H100 with vLLM batched inference).
Explanations are generated once and cached; subsequent embedding and kNN computation take $<$5 minutes.
For larger datasets, selective explanation (only for high-entropy examples) could reduce cost.
...
We evaluated exclusively on SST-2 sentiment classification.
While SST-2 is a standard benchmark, generalization to other domains (e.g., NLI, question answering, named entity recognition) and languages remains to be demonstrated.
...
We report results from single experimental runs without error bars or confidence intervals.
While our main findings show large effect sizes (e.g., 0.832 vs 0.107 AUROC), future work should include multiple runs with different random seeds to quantify variance.
...
Extension to multi-class and structured prediction tasks requires adapting the graph construction and scoring mechanisms.
